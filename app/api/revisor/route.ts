...
import { revisorSchema, vazioParaNull } from "@/lib/validation";
import { createSupabaseAnonClient } from "@/lib/supabase/anon";
import { checkRateLimit, getClientIp } from "@/lib/ratelimit";

// Inscrição de revisores. Caminho de escrita público, sem service role.
export async function POST(req: Request) {
  ...
  const parsed = revisorSchema.safeParse(body);
  if (!parsed.success) {
  ...
  // Honeypot: bot preencheu o campo invisível -> aceita silenciosamente e descarta
  ...
  if (!permitido) {
    return NextResponse.json(
      { erro: "Muitas inscrições em pouco tempo. Aguarde um instante e tente de novo." },
      { status: 429 }
    );
  }
